...
public class Parser {
	...
	public void parse(BufferedReader reader, DlvIOHandler dlvIOHandler) {
		this.parsingTime = System.currentTimeMillis();
		this.dm = dlvIOHandler;
		int i = 0;
		for (int j = 0; (j < dlvIOHandler.getInvocationParameters().length) && (i == 0); ++j) {
			if ((!dlvIOHandler.getInvocationParameters()[j].equals("-FB")) && (!dlvIOHandler.getInvocationParameters()[j].equals("-FC"))
					&& (!dlvIOHandler.getInvocationParameters()[j].equals("-cautious")) && (!dlvIOHandler.getInvocationParameters()[j].equals("-brave")))
				continue;
			i = 1;
		}
		if (i != 0)
			startQueryParser(new InputHandler(reader));
		else
			startDatalogParser(new InputHandler(reader));
		this.dm = null;
	...
	private void addPredicate(Model paramModel, String paramString, ArrayList paramArrayList, boolean paramBoolean) {
		...
		if (localPredicate == null) {
			localPredicate = this.dm.getOutputDescriptor().getDescriptor(paramString).buildPredicate(paramString, paramArrayList.size(), paramModel,
					this.modelNumber);
		...
		localPredicate.addLiteral(tmp54_52.new Literal(Tools.arrayListToStringArray(paramArrayList), paramBoolean));
	}
 ...
 * DLV.Parser JD-Core Version: 0.5.4
 */