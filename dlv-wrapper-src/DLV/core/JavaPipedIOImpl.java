  ...
  implements AbstractIOImplementation
{
  ...
  public synchronized void run(DlvIOHandler paramDlvIOHandler)
    throws DLVInvocationException
  {
    reset();
    try
    {
      String[] arrayOfString = new String[paramDlvIOHandler.getInvocationParameters().length + 1];
      for (int i = 0; i < paramDlvIOHandler.getInvocationParameters().length; ++i)
        arrayOfString[i] = paramDlvIOHandler.getInvocationParameters()[i];
      arrayOfString[paramDlvIOHandler.getInvocationParameters().length] = "--";
      arrayOfString[0] = this.dlvPath;
      this.dlv = this.runtime.exec(arrayOfString);
      this.status = 1;
      this.input = new BufferedReader(new InputStreamReader(this.dlv.getInputStream(), paramDlvIOHandler.getEncoding()));
      this.error = new BufferedReader(new InputStreamReader(this.dlv.getErrorStream(), paramDlvIOHandler.getEncoding()));
      this.output = new PrintWriter(new OutputStreamWriter(this.dlv.getOutputStream(), paramDlvIOHandler.getEncoding()));
      this.output.print(paramDlvIOHandler.getProgram().getProgramStrings().toString());
      Predicate[] arrayOfPredicate = paramDlvIOHandler.getProgram().getPredicates();
      if (arrayOfPredicate != null)
        for (int j = 0; j < arrayOfPredicate.length; ++j)
          arrayOfPredicate[j].appendTo(this.output);
      this.output.print(' ');
      this.output.close();
      this.output = null;
      this.warningReader = new WarningReader(null);
      this.warningReader.start();
      this.inputReader = new InputReader(paramDlvIOHandler);
  ...
  public synchronized void kill(DlvIOHandler paramDlvIOHandler)
    throws DLVInvocationException
  {
  ...
  public void setWarnings(String paramString)
    throws DLVInvocationException
  {
    synchronized (this.warnings)
    {
  ...
  public void freeze(DlvIOHandler paramDlvIOHandler)
    throws DLVInvocationException
  {
    throw new UnsupportedOperationException("freeze feature not implemented");
  }

  public void deFreeze(DlvIOHandler paramDlvIOHandler)
    throws DLVInvocationException
  {
  ...
  private class WarningReader extends Thread
  {
    ...
    public void run()
    {
      ...
      {
        String str = "";
        while (((str = this.this$0.error.readLine()) != null) && (!interrupted()))
          try
          {
            this.this$0.setWarnings(str);
          }
      ...
      catch (Throwable localThrowable)
      {
        try
        {
          this.this$0.setWarnings("Error Type: " + localThrowable.getClass() + " Message: " + localThrowable.getMessage());
        }
 ...
 * Qualified Name:     DLV.core.JavaPipedIOImpl
