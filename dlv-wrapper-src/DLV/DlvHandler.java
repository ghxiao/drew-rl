  ...
  implements DlvIOHandler
{
  ...
  public synchronized void run(byte paramByte)
    throws DLVInvocationException
  {
  ...
  public synchronized void deFreeze()
    throws DLVInvocationException
  {
    if (this.status != 4)
      return;
    ...
    throws DLVInvocationException
  {
    ...
    if ((this.status == 1) || (this.status == 4))
      ...
      {
        label142: throw new DLVInvocationException(localInterruptedException.getMessage());
      }
    if (this.error != null)
