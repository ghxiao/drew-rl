...
/**
 * The heart of InterProlog; a PrologEngine represents a Prolog machine
 * instance. This is an abstract class; you should use it just to declare
 * variables, but must instantiate only subclasses.
 */
public abstract class AbstractPrologEngine implements PrologEngine {
	...
	/**
	 * File path to directory with Prolog machine, or command path, which may
	 * have options appended
	 */
	...
	/**
	 * Temporary directory where Prolog files are unjared to. It is
	 * automatically declared as a Prolog library_directory
	 */
	protected File tempDirectory;

	/**
	 * Table of object references that can be referred from Prolog without being
	 * serialized
	 */
	...
	/**
	 * InterProlog startup files should be loaded from the jar file, rather than
	 * from classname-relative file locations
	 */
	...
	/**
	 * Threads currently having a pending deterministicGoal computation on the
	 * Prolog side
	 */
	protected Stack dgThreads = new Stack();

	/**
	 * Prolog Goals whose execution has not yet finished or whose results have
	 * not yet been returned to their Java clients
	 */
	Vector goalsToExecute;
	/**
	 * javaMessage requests that have started execution and whose results have
	 * not yet been returned to Prolog.
	 */
	Vector messagesExecuting;

	/**
	 * The thread that handles javaMessage callbacks, and deterministicGoal
	 * results
	 */
	...
	/**
	 * Create a Prolog executor, possibly spawning it in a different process or
	 * loading it into memory, depending on the implementation by our subclass.
	 * 
	 * @param prologBinDirectoryOrCommand
	 *            File path to Prolog machine, see subclass docs for precise
	 *            semantics
	 * @param debug
	 *            if true, print progress messages to aid debugging
	 * @param loadFromJar
	 *            if true, startup files should be loaded from the jar file,
	 *            rather than directly from the file system
	 *            <p>
	 *            IF prologBinDirectoryOrCommand is null then InterProlog needs
	 *            to find values for one or more properties, XSB_BIN_DIRECTORY
	 *            and/or SWI_BIN_DIRECTORY, depending on which Prolog(s) you're
	 *            using.
	 *            <p>
	 *            First it looks for an 'interprolog.defs' file (a Properties
	 *            file in the format described in
	 *            http://java.sun.com/j2se/1.4.2/
	 *            docs/api/java/util/Properties.html, in the directory where the
	 *            jar file containing the InterProlog classes is). <br>
	 *            If 'interprolog.defs' does not exist, an attempt is made to
	 *            get the property through System.getProperties(); in this
	 *            scenario you can define the property by using the -D java
	 *            (command) switch
	 */
	public AbstractPrologEngine(String prologBinDirectoryOrCommand,
			boolean debug, boolean loadFromJar) {
				...
				try {
					...
					if (prologBinDirectoryOrCommand == null)
						throw new IPException(
								"Bad ..._BIN_DIRECTORY in interprolog.defs file:"
										+ prologBinDirectoryOrCommand);
					prologBinDirectoryOrCommand = executablePath(prologBinDirectoryOrCommand);
				} catch (IOException e) {
					throw new IPException(
							"Could not read interprolog.defs file");
				}
			} else
				prologBinDirectoryOrCommand = executablePath(getBinDirectoryProperty(System
						.getProperties()));
		}
		if (prologBinDirectoryOrCommand == null)
		...
		makeTempDirectory(); // not just for InterProlog startup! That's why
								// this is NOT conditional
		knownObjects = new ObjectRegistry();
		goalsToExecute = new Vector();
		messagesExecuting = new Vector();
		try {
			getRealJavaObjectMethod = findMethod(getClass(),
					"getRealJavaObject", new Class[] { Object.class });
		} catch (Exception ex) {
			throw new IPException(
					"could not find special getRealJavaObject method:" + ex);
		}
	}

	protected void loadInitialFile() {
		progressMessage("Setting up Prolog library paths...");
		String tempPath = peer.unescapedFilePath(tempDirectory
				.getAbsolutePath());
		command("assertz(library_directory('" + tempPath + "'))"); // used to be
																	// asserta
																	// until Aug
																	// 2005
		...
		if (loadFromJar)
			consultFromPackage(F, AbstractPrologEngine.class);
		else
			consultRelative(F, AbstractPrologEngine.class);
	}
	...
	/**
	 * Returns the Prolog numeric version for this engine; useful for
	 * functionality that depends on features of a particular version. It is
	 * requested to Prolog dynamically, but only once. Should not be used only
	 * after the engine objects is initialized (constructed).
	 */
	...
	/**
	 * Returns the installation directory for the Prolog system, without the
	...
	/**
	 * Computes the installation directory for the Prolog system, without the
	 * trailing separator char
	 */
	...
	public static boolean isWindowsOS() { // if we're under Windows --
											// isWindowsOS
	...
	public void shutdown() {
		...
		shutingDown = true;
		abortTasks();
		// prologHandler.interrupt(); // cannot do this here, in NativeEngine
		// crashes the whole JVM
	}
	...
	/**
	 * Give the OS another try at creating the temporary directory; although
	 * very rarely, sometimes Windows fails with no reason
	 */
	void makeTempDirectory(int tries) {
		...
		try {
			...
			if (tries > 0) {
				...
				throw new IPException("Problems creating temporary directory:"
						+ e);
		}
	}
	...
	File copyToTemp(String filename, Object requester) {
		...
		// if (filename.indexOf('.')==-1) throw new
		// IPException("File name missing extension");
		...
		try {
			...
			if (!(filename.indexOf('.') == -1)) {
				...
				resourceStream = rcl.getResourceAsStream(resourceName
						+ filename);
			} else {
			...
			if (resourceStream == null)
				throw new IPException("Resource not found:" + resourceName
						+ newfilename);
			...
			if (parentDir != null && !parentDir.exists()) {
				...
				// code below added by C. Rojo, June 13th. Modified to make any
				// additional
				...
				// end of C. Rojo added code. See also
				// setTempDirectoriesToDelete
			}
			...
			progressMessage("Created file output stream in tempDirectory:"
					+ fos);
			...
			throw new IPException("I/O problem obtaining " + newfilename + ": "
					+ e);
		}
	 ...
	 * @param nestedTempDirectories
	 *            Directory path containing new directories to set for deletion.
	 */
	private void setTempDirectoriesToDelete(File nestedTempDirectories) {
		if (nestedTempDirectories != null
				&& !nestedTempDirectories.getAbsolutePath().equals(
	...
	/**
	 * Some Prologs use '\' as an escape character in atoms, which can affect
	 * file paths under Windows. Use this method to preprocess all file paths
	 * passed to Prolog.
	 */
	...
	protected static String currentOSpath(String javaPath) {
		...
		for (int c = 0; c < sb.length(); c++)
			if (sb.charAt(c) == '/')
				sb.setCharAt(c, File.separatorChar);
		return sb.toString();
	}

	/**
	 * Given a Prolog filename without extension, return it completed with all
	...
	/**
	 * Extracts a Prolog file from the jar file or directory where the
	 * requester's class came from, and asks Prolog process to consult it. You
	 * should use this method only after your program is stable. The Prolog file
	 * is extracted into a temporary file, that is automatically deleted on
	 * exiting the application.
	 * 
	 * @param filename
	 *            The Prolog file name, including extension; if it has no
	 *            extension, the Prolog file extensions are appended in turn
	 *            until a file is found
	 * @param requester
	 *            Defines where the Prolog file resides
	...
	/**
	 * Consults a Prolog file from the directory where the requester's class
	 * would come from if it did not come from a jar file. Adds that directory
	 * to the library_directory relation, so modules can be found there
	 * 
	 * @param filename
	 *            The Prolog file name, including suffix; if a path it should
	 *            use '/' as the separator, independently of the OS
	 * @param requester
	 *            Defines where the Prolog file resides
	 */
	...
	protected void operationRelative(String operation, String filename,
			Object requester) {
		Class rc;
		if (requester instanceof Class)
			rc = (Class) requester;
			...
			rc = requester.getClass();
		String path = rc.getPackage().getName()
				.replace('.', File.separatorChar);
		path = cleanPath(getJarDirectory().getPath()) + File.separatorChar
				+ path;
		...
		String g = "((library_directory('" + path
				+ "')->true;assert(library_directory('" + path + "'))), "
				+ operation + "('" + filename + "'))";
		if (!command(g))
	 ...
	 * necessary, only for Java 1.4 and later. From java version 1.4 paths
	 * returned by File.getPath contain %20 instead of spaces
	 */
	private String cleanPath(String p) {
		progressMessage("Cleaning path " + p);
		if (System.getProperty("java.version").compareTo("1.4") >= 0) {
			try {
				Method decode = URLDecoder.class.getMethod("decode",
						new Class[] { String.class, String.class });
				return (String) decode.invoke(new URLDecoder(), new Object[] {
						p, "UTF-8" });
			} catch (Exception e) {
				throw new IPException("Inconsistency in PrologEngine.cleanPath"
						+ e);
			}
		} else
			return p;
	...
	/**
	 * Interrupt Prolog and make it return to its top level. This is the
	 * equivalent to performing a ctrl+c or similar command when using Prolog
	 * under a standard console shell.
	 */
	...
	public boolean command(String s) {
		if (topGoalHasStarted)
			return deterministicGoal(s);
		else
			return realCommand(s);
	}

	/**
	 * Implementation of a simple parameterless Prolog goal; does not support
	 * recursive nor multithreaded operation, use command instead
	 * 
	 * @see #command(String)
	 */
	 ...
	 * ipIsDebugging/0. Beware that this may try to assert/retract a flag
	 * (ipIsDebugging/0) on the Prolog side, be sure to invoke this method
	 * sooner rather than later as it may get "stuck" if there are problems
	 * communicating with the Prolog engine
	 */
	public void setDebug(boolean d) {
		if (d)
			System.out.println(System.currentTimeMillis() - startTime + "ms: "
					+ "now showing debugging information");
	...
	public static void printBindings(Object[] b) {
		if (b == null)
			...
			for (int i = 0; i < b.length; i++)
				System.out.println("Binding " + i + ":" + b[i]);
	}

	protected final void teachIPobjects(ObjectOutputStream obs)
			throws IOException {
		Object[] oa1 = { new Integer(13) };
		obs.writeObject(new ObjectExamplePair("GoalFromJava", new GoalFromJava(
				1, "a", "aa", new Object[0], "A"), new GoalFromJava(2, "b",
				"bb", oa1, "B")));
		obs.writeObject(new ObjectExamplePair("ResultFromProlog",
				new ResultFromProlog(1, true, 0, null), new ResultFromProlog(2,
						false, 1, "some error")));
		...
		obs.writeObject(new ObjectExamplePair("MessageFromProlog", mpA, mpB));
		/* crashed JRE 1.4 with new Exception(): */
		obs.writeObject(new ObjectExamplePair("ResultFromJava",
				new ResultFromJava(1, null, null, new Object[0]),
				new ResultFromJava(2, "result here", "new Exception()...",
						new Object[1])));
		obs.writeObject(new ObjectExamplePair("InvisibleObject",
				new InvisibleObject(1), new InvisibleObject(2)));
		obs.writeObject(new ObjectExamplePair("IPClassObject",
				new IPClassObject("A"), new IPClassObject("B")));
		obs.writeObject(new ObjectExamplePair("IPClassVariable",
				new IPClassVariable("ClassA", "VariableA"),
				new IPClassVariable("ClassB", "VariableB")));
	}

	/**
	 * Provide Prolog with material to construct special objects that actually
	 * represent basic type values, so that Prolog can call (through
	 * javaMessage) methods with basic type (non-object) arguments. Also
	 * provides material for some other convenience objects
	 */
	protected void teachBasicObjects(ObjectOutputStream obs) throws IOException {
		obs.writeObject(new ObjectExamplePair("boolean", new BasicTypeWrapper(
				new Boolean(true)), new BasicTypeWrapper(new Boolean(false))));
		obs.writeObject(new ObjectExamplePair("byte", new BasicTypeWrapper(
				new Byte((new Integer(1)).byteValue())), new BasicTypeWrapper(
				new Byte((new Integer(2)).byteValue()))));
		obs.writeObject(new ObjectExamplePair("char", new BasicTypeWrapper(
				new Character('A')), new BasicTypeWrapper(new Character('B'))));
		obs.writeObject(new ObjectExamplePair("double", new BasicTypeWrapper(
				new Double(1)), new BasicTypeWrapper(new Double(2))));
		obs.writeObject(new ObjectExamplePair("float", new BasicTypeWrapper(
				new Float(1)), new BasicTypeWrapper(new Float(2))));
		obs.writeObject(new ObjectExamplePair("int", new BasicTypeWrapper(
				new Integer(1)), new BasicTypeWrapper(new Integer(2))));
		obs.writeObject(new ObjectExamplePair("long", new BasicTypeWrapper(
				new Long(1)), new BasicTypeWrapper(new Long(2))));
		obs
				.writeObject(new ObjectExamplePair("short",
						new BasicTypeWrapper(new Short((new Integer(1))
								.shortValue())), new BasicTypeWrapper(
								new Short((new Integer(2)).shortValue()))));
		obs.writeObject(new ObjectExamplePair(new Boolean(true), new Boolean(
				false)));
		obs.writeObject(new ObjectExamplePair(new Integer(1), new Integer(2)));
		obs
				.writeObject(new ObjectExamplePair(new Float(20.59375),
						new Float(2)));
		obs.writeObject(TermModel.example());
		obs.writeObject(VariableNode.example());
		obs.writeObject(new ObjectExamplePair("ArrayOfTermModel",
				new TermModel[0], new TermModel[1]));
	...
	/**
	 * Send an array of object example pairs to Prolog and generate ipObjectSpec
	 * facts. Returns true if this succeeds, false otherwise.
	 * 
	 * @param examples
	 *            The examples
	...
	public boolean teachMoreObjects(ObjectExamplePair[] examples) {
		...
		for (int i = 0; i < examples.length; i++)
			temp[i] = examples[i];
		return deterministicGoal("ipProcessExamples(Examples)", "Examples",
				temp);
	}
	...
	protected GoalFromJava makeDGoalObject(String G, String OVar,
			Object[] objectsP, String RVars, int timestamp) {
		...
		if (objectsP == null)
			...
			// safeguard against typical usage mistake, may however be too
			// strict...
			throw new IPException(
					"objectsP argument must be an array of Object");
		if (G.trim().endsWith("."))
			throw new IPException(
					"Goal argument should have no trailing '.', in deterministicGoal");
		return new GoalFromJava(timestamp, G, OVar, objectsP, RVars);
	}

	/**
	 * Synchronously calls a Prolog goal. Only the first solution is considered.
	 * G should contain a syntactically correct Prolog term, without the
	 * trailing dot (.). Throws an IPAbortedException if a Prolog abort happens,
	 * and an IPInterruptedException if the interrupt() method was invoked.
	 * 
	 * @see #deterministicGoal(String)
	 * @see #deterministicGoal(String,String)
	 * @see #deterministicGoal(String,String,Object[])
	 * @return a new array containing an object for each term in the rVars list,
	 *         or null if goal fails
	 * @param G
	 *            Prolog goal term
	 * @param OVar
	 *            Prolog variable that will be bound to objectsP array
	 * @param objectsP
	 *            Array of Java objects to pass to Prolog goal
	 * @param RVars
	 *            Prolog list with object specifications, typically containing
	 *            variables occurring in g. If null a single binding will be
	 *            returned, containing a TermModel object representing the goal
	 *            term solution
	 */
	public Object[] deterministicGoal(String G, String OVar, Object[] objectsP,
			String RVars) {
		// available=false;
		...
		try {
			progressMessage("Schedulling (in PrologEngine) goal " + G
					+ ", timestamp " + mytimestamp + " in thread "
					+ Thread.currentThread().getName());
			GoalToExecute goalToDo = new GoalToExecute(GO, Thread
					.currentThread());
			...
			if (goalToDo.wasAborted())
				throw new IPAbortedException(G
						+ " was aborted by Java-side cascading");
			if (goalToDo.wasInterrupted())
				throw new IPInterruptedException(G
						+ " was interrupted by Java-side cascading");
			if (result.wasInterrupted(this))
				throw new IPInterruptedException(G
						+ " was interrupted, Prolog detected");
			if (result.error != null)
				throw new IPPrologError(result.error);
			if (result.timestamp != mytimestamp)
				throw new IPException(
						"bad timestamp in deterministicGoal, got "
								+ result.timestamp + " instead of "
								+ goalTimestamp);
			if (result.succeeded)
	...
	/**
	 * A parameterless goal with no result other than success/failure. Same as
	 * deterministicGoal(G, null,null,"[]")
	 */
	...
	/**
	 * Useful when you're constructing objects from Prolog, but don't need to
	 * pass any from Java. Same as deterministicGoal(G,null,null,RVars)
	 */
	...
	/**
	 * Useful when you want to pass objects to Prolog but don't need objects
	 * returned. Same as deterministicGoal(G, OVar,objectsP,"[]")
	 */
	 ...
	 * goal G in another PrologEngine E: buildTermModel(G,GM),
	 * javaMessage(E,SM,deterministicGoal(GM)), recoverTermModel(SM,Solution)
	 */
	public TermModel deterministicGoal(TermModel G) {
		Object[] bindings = deterministicGoal(
				"recoverTermModel(GM,G), call(G), buildTermModel(G,SM)",
				"[GM]", new Object[] { G }, "[SM]");
		if (bindings == null)
			return null;
		else
	...
	protected int incGoalTimestamp() {
		...
		if (goalTimestamp < 0)
			throw new IPException(
					"goalTimestamp did wrap around, please improve it...");
	...
	protected synchronized GoalToExecute findLastGTEWithProperThread() {
		for (int i = goalsToExecute.size() - 1; i >= 0; i--) {
			...
			if (gte.hasStarted() && !gte.hasEnded()
					&& gte.getCallerThread() == currentDGthread())
				return gte;
		}
		throw new IPException(
				"Could not find thread for callback; currentDGthread=="
						+ currentDGthread() + "; " + goalsToExecute.size()
						+ " GTEs");
	}
	...
	/**
	 * Currently does a dumb linear search, enough for our scenarios. 2 future
	 * possibilities: use Hashtable; make ResultFromProlog bring back a
	 * reference to GoalFromJava
	 */
	...
	protected synchronized MessageExecuting lastMessageRequest() {
		...
		if (messagesExecuting.size() == 0)
			last = null;
		else
			last = (MessageExecuting) messagesExecuting.lastElement();
		return last;
	}
	...
	public synchronized void endAllTasks(Exception e) {
		for (int i = 0; i < goalsToExecute.size(); i++) {
			GoalToExecute gte = (GoalToExecute) goalsToExecute.elementAt(i);
			gte
					.setResult(new ResultFromProlog(gte.getTimestamp(), false,
							0, e));
		}
		cleanupTasks();
	}
	...
	/**
	 * Do not invoke this. Due to the use of several Java packages in
	 * InterProlog this method must be qualified as public
	 */
	 ...
	 * Subclass implementations should return false if the Prolog engine is not
	 * available for work, e.g. prompt was not yet recognized in a subprocess
	 * (socket) engine
	 */
	...
	/**
	 * Sleeps the current Java thread until this engine is idle. If this never
	...
	/**
	 * Handling of javaMessages and deterministicGoals. This is where most
	 * things happen.
	 * 
	 * @param x
	 *            Argument of the callback predicate
	 */
	public Object handleCallback(Object x) {
		...
		if (x instanceof ClassNotFoundException)
			...
			if (gte == null)
				throw new IPException("Could not find goal " + rfp.timestamp);
			gte.setResult(rfp);
		} else
			throw new IPException("bad object in handleCallback:" + x);
		// no errors so far
	...
	protected Object doSomething() {
		while (!shutingDown) {
			// let some work proceed elsewhere; this seems light enough for now,
			// if not use wait/notify around here too:
			if (isIdle())
				try {
					Thread.sleep(0, 100);
				} catch (InterruptedException e) {
				...
				// Thread.yield(): too heavy, for ex. when in a modal dialog
				try {
					Thread.sleep(0, 1);
				} catch (InterruptedException e) {
					throw new IPException("Bad interrupt:" + e);
	...
	private boolean isFirstJavaMessage(MessageFromProlog mfp) {
		if (!mfp.methodName.equals(firstJavaMessageName))
			return false;
		if ((mfp.target instanceof InvisibleObject)
				&& getRealJavaObject((InvisibleObject) mfp.target) == this)
			return true;
		else
			return false;
	}
	...
	public final void firstJavaMessage() {
		throw new IPException(
				"This should never be called, bad javaMessage handling");
	}
	...
	public ResultFromJava doCallback(Object x) {
		...
		try {
			if (callback.target instanceof InvisibleObject) {
				...
				target = Class
						.forName(((IPClassObject) (callback.target)).classname);
				...
				target = tempClass.getField(tempTarget.variableName).get(
						tempClass);
			} else
			...
			for (int a = 0; a < formalArguments.length; a++) {
				...
				if (localArguments[a] != null) {
					if (localArguments[a] instanceof BasicTypeWrapper) {
						...
						localArguments[a] = btw.wrapper; // Integer instead of
															// int ???
						...
						localArguments[a] = Class
								.forName(((IPClassObject) (localArguments[a])).classname);
						...
						localArguments[a] = tempClass.getField(
								IPCV.variableName).get(null);
						formalArguments[a] = localArguments[a].getClass();
					} else
						formalArguments[a] = localArguments[a].getClass();
				}
			...
			if (target instanceof Class) {
				if (shortClassName((Class) target).equals(callback.methodName)) {
					...
					// ((Class)target).getConstructor(formalArguments);
					Constructor constructor = findConstructor((Class) target,
							formalArguments);
					result = constructor.newInstance(localArguments);
				} else {
					// It's a class (static) method invocation
					// Method method =
					// ((Class)target).getMethod(callback.methodName,formalArguments);
					method = findMethod((Class) target, callback.methodName,
							formalArguments);
				...
				// Method method =
				// target.getClass().getMethod(callback.methodName,formalArguments);
				// result =
				// findMethod(target.getClass(),callback.methodName,formalArguments).invoke(target,localArguments);
				method = findMethod(target.getClass(), callback.methodName,
						formalArguments);
				result = method.invoke(target, localArguments);
			}
			// The result will be an invisible object, except if a String or a
			// wrapper or a TermModel...
			// ...or if this is a getRealJavaObject message sent to the
			// PrologEngine
			if (result != null
					&& !(target == this && method
							.equals(getRealJavaObjectMethod))
					&& !(result instanceof InvisibleObject)
					&& !(result instanceof String)
					&& !(result instanceof TermModel)
		...
		if (callback.returnArguments)
			return new ResultFromJava(callback.timestamp, result, exception,
					callback.arguments);
		else
			return new ResultFromJava(callback.timestamp, result, exception,
					null);
	 ...
	 * javaMessage predicate with method argument polimorphism. If the type
	 * signatures do not match exactly, searches all method signatures to see if
	 * their arguments are type-compatible.
	 */
	public static Method findMethod(Class targetClass, String name,
			Class[] formalArguments) throws NoSuchMethodException {
		Method m = null;
		try {
			...
			for (int i = 0; i < allMethods.length; i++) {
				if (allMethods[i].getName().equals(name)
						&& allMethods[i].getParameterTypes().length == formalArguments.length) {
					boolean compatible = true;
					for (int j = 0; j < formalArguments.length; j++) {
						if (!assignableType(
								allMethods[i].getParameterTypes()[j],
								formalArguments[j])) {
							compatible = false;
							break;
						}
			...
			if (m == null) {
				...
				System.err.println("Could not find " + name + " in "
						+ targetClass);
		...
		try {
			...
			for (int i = 0; i < allConstructors.length; i++) {
				if (allConstructors[i].getParameterTypes().length == formalArguments.length) {
					boolean compatible = true;
					for (int j = 0; j < formalArguments.length; j++) {
						if (!assignableType(allConstructors[i]
								.getParameterTypes()[j], formalArguments[j])) {
							compatible = false;
							break;
						}
					}
	...
	/**
	 * Returns just the name of the class, with no package information. That is,
	 * if <CODE>foo.bar.Mumble</CODE> were the class passed in, the string
	 * <CODE>"Mumble"</CODE> would be returned. Similarly, if the class that is
	 * passed in is <CODE>a.b.Class$InnerClass</CODE> the string returned would
	 * be <CODE>InnerClass</CODE>.
	 */
	public static String shortClassName(Class c) {
		...
		if (dot == -1 && dollar == -1)
			...
			return s.substring(Math.max(dot, dollar) + 1, s.length());
	}

	/**
	 * Register an object with this Engine, so it later can be referred from
	 * Prolog without serializing it.
	 * 
	 * @param x
	 *            Object to be registered
	 * @return Integer denoting the object. In Prolog one can then refer to it
	 *         by using the InvisibleObject class.
	 * @see InvisibleObject
	 */
	...
	/**
	 * Register an object with this Engine, so it later can be referred from
	 * Prolog without serializing it, and returns an InvisibleObject
	 * encapsulating the reference.
	 * 
	 * @param x
	 *            Object to be registered
	 * @return InvisibleObject denoting the object. In Prolog one can then refer
	 *         to it by using the InvisibleObject class.
	 * @see InvisibleObject
	 */
	...
	/**
	 * Just returns the object, untouched (but "dereferenced" if called from
	 * Prolog). This serves the need to get objects in javaMessage because of
	 * the way CallbackHandler.doCallback works. For example:
	 * ipPrologEngine(_E), stringArraytoList(_O,[miguel,calejo]),
	 * javaMessage(_E,_R,getRealJavaObject(_O)),stringArraytoList(_R,List). ...
	 * will bind List to [miguel,calejo] and not to an InvisibleObject
	 * specification as ordinarly would happen
	 */
	 ...
	 * used with extreme caution since any further prolog calls to the object by
	 * means of reference to it in the registry might result in unpredictable
	 * behaviour.
	 */
	...
	/**
	 * Removes references to objects of class <code>cls</code> from the
	 * registry. This method should be used with extreme caution since any
	 * further prolog calls to the unregistered objects by means of reference to
	 * them in the registry might result in unpredictable behaviour.
	 */
	...
	/**
	 * If true, the Java execution of javaMessage predicates will happen in new
	 * threads (default); if false, execution will be under the thread of the
	 * deterministicGoal currently executing in Prolog
	 */
