...
/**
 * A PrologEngine implemented over TCP/IP sockets. A SubprocessEngine object
 * represents and gives access to a running Prolog process in background.
 * Multiple instances correspond to multiple Prolog processes, outside the Java
 * Virtual Machine.
 */
public abstract class SubprocessEngine extends AbstractPrologEngine {
	...
	Socket intSocket = null; // Used only for a particular way of interrupting
								// Prolog
	String interruptCommand = null; // Used only for UNIX
	Vector listeners = new Vector();
	protected boolean available;

	static class ClientRecognizer extends Recognizer implements
			RecognizerListener {
	...
	/**
	 * Add a PrologOutputListener to this engine. All stdout and stderr output
	 * will be routed to the client.
	 * 
	 * @param client
	 *            An object interested in receiving messages depicting Prolog's
	 *            progress
	...
	public synchronized void removePrologOutputListener(
			PrologOutputListener client) {
	...
	/**
	 * Add a OutputListener to get output from Prolog's standard output. This is
	 * a lower level interface than
	 * addPrologOutputListener(PrologOutputListener).
	 * 
	 * @param client
	 *            An object interested in Prolog's standard output
	 ...
	 * 
	 * @param prologCommand
	 *            The command to launch Prolog, as if given from a console
	 *            shell. Must not be null.
	 * @param debug
	 *            If true this engine will send debugging messages to System.out
	...
	public SubprocessEngine(String prologCommand, boolean debug,
		...
		// Let's make sure PrologEngines get their finalize() message when we
		// exit
		if (System.getProperty("java.version").compareTo("1.3") >= 0) {
			...
			try {
				Method finalizeOnExit = System.class.getMethod(
						"runFinalizersOnExit", new Class[] { boolean.class });
				finalizeOnExit.invoke(null, new Object[] { new Boolean(true) }); // for
																					// static
																					// methods
																					// first
																					// arg
		...
		try {
			RecognizerListener availableSetter = new RecognizerListener() {
				public void recognized(Recognizer source, Object extra) {
					available = true;
					progressMessage("I'm available! source:" + source
							+ " extra:" + extra);
				}
			};
			...
			String myHost = "127.0.0.1"; // to avoid annoying Windows dialup
											// attempt
			...
			command("ipinitialize('" + myHost + "',"
					...
					+ registerJavaObject(this) + "," + debug + ")");
			progressMessage("Waiting for the socket to accept...");
			socket = serverSocket.accept();

			progressMessage("Teaching examples to Prolog...");
			PrologOutputObjectStream bootobjects = buildPrologOutputObjectStream(socket
					.getOutputStream());
			...
			waitUntilAvailable();
			setupCallbackServer();
			prepareInterrupt(myHost); // OS-dependent Prolog interrupt
										// generation, must be after the
										// previous step
			waitUntilAvailable();
	...
	protected PrologOutputObjectStream buildPrologOutputObjectStream(
		...
		prologHandler = new Thread() {
			public void run() {
				try {
					...
					// If this happens, it means there was a communications
					// error
					// with prolog. We have to abort all current goals so that
					// calls are not wait()-ing forever.
					if (!shutingDown) {
						IPException toThrow = new IPException(
								"Bad exception in setupCallbackServer", e);
	...
	protected Object receiveObject() throws IOException {
		...
		Object x = null;
		try {
			ObjectInputStream ios = new ObjectInputStream(socket
					.getInputStream());
			x = ios.readObject();
	...
	protected void sendObject(Object y) throws IOException {
		...
		PrologOutputObjectStream poos = buildPrologOutputObjectStream(socket
				.getOutputStream());
		poos.writeObject(y);
	...
	/**
	 * Shuts down the background Prolog process as well as the dependent Java
	 * threads.
	 */
	...
	/**
	 * Kill the Prolog background process. If you wish to make sure this message
	 * is sent on exiting, use System.runFinalizersOnExit(true) on
	 * initialization
	 */
	...
	protected void setDetectPromptAndBreak(boolean yes) {
		if (yes == isDetectingPromptAndBreak())
			return;
		if (yes) {
	...
	protected boolean isDetectingPromptAndBreak() {
		return stdoutHandler.hasListener(promptTrigger) /*
														 * &&
														 * stderrHandler.hasListener
														 * (promptTrigger)
														 */
				&& stdoutHandler.hasListener(breakTrigger) /*
															 * &&stderrHandler.
															 * hasListener
															 * (breakTrigger)
															 */;
	...
	/**
	 * Sends a String to Prolog's input. Its meaning will naturally depend on
	 * the current state of Prolog: it can be a top goal, or input to an ongoing
	 * computation
	 */
	...
	protected void prepareInterrupt(String myHost) throws IOException { // requires
																		// successful
																		// startup
																		// steps
		if (isWindowsOS()) {
			intServerSocket = new ServerSocket(0);
			command("setupWindowsInterrupt('" + myHost + "',"
					+ intServerSocket.getLocalPort() + ")");
			intSocket = intServerSocket.accept();
		} else {
			// available=true; // sort of a hack... but when will the state of
			// 'available' become valid ?
			waitUntilAvailable();
			Object bindings[] = deterministicGoal(
					"getPrologPID(N), ipObjectSpec('java.lang.Integer',Integer,[N],_)",
					"[Integer]");
			progressMessage("Found Prolog process ID");
			if (bindings != null)
				interruptCommand = "/bin/kill -s INT " + bindings[0];
			else
	...
	/**
	 * This implementation may get stuck if the command includes variables,
	 * because the Prolog top level interpreter may offer to compute more
	 * solutions; use variables prefixed with '_'
	 */
	...
	/**
	 * Very alike deterministicGoal except that it sends the initial
	 * GoalFromJava object over the socket
	 */
	protected Object[] firstGoal(String G, String OVar, Object[] objectsP,
			String RVars) {
		...
		try {
			GoalFromJava GO = makeDGoalObject(G, OVar, objectsP, RVars,
					mytimestamp);
			progressMessage("Prepared GoalFromJava:" + GO);

			progressMessage("Schedulling (first) goal " + G + ", timestamp "
					+ mytimestamp + " in thread "
			...
			topGoalHasStarted = false; // is this OK? this assumes no initiative
										// from the Prolog side, which is
										// probably correct
			// removeErrorHandling();
			progressMessage("Leaving firstGoal for " + G + ", timestamp "
					+ mytimestamp + " isIdle()==" + isIdle());
		}
	...
	protected synchronized boolean onlyFirstGoalSchedulled() {
		return isIdle()
				|| (messagesExecuting.size() == 0 && goalsToExecute.size() == 1 && ((GoalToExecute) goalsToExecute
						.elementAt(0)).isFirstGoal());
	}
	...
	protected void setupErrorHandling() {
		...
		stderrHandler.addOutputListener(errorTrigger); // no need to listen to
														// stdout
		abortMessage = "";
	...
	Recognizer errorTrigger = new Recognizer("++Error", true); // was
																// "++Error: "
																// for XSB 2.4
